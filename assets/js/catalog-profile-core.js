    ...
    renderPrintableProfileHtml(setCode, setName, cards, options) {
      ...
      for(let pageIndex=0;pageIndex<totalPages;pageIndex++){
        ...
        const cells=slice.map((card,offset)=>{
          ...
          const artBoxMode = ps.microArtBoxMode === 'custom' ? 'custom' : 'flow';
          ...
          const microArt = options.imageMode === 'art-crop' && card._artCropImage
            ? (()=>{ const zoom=1+(Number(ps.microArtZoom||0)/100)*1.2; const x=Math.max(0,Math.min(100,Number(ps.microArtPositionX==null?50:ps.microArtPositionX))); const y=Math.max(0,Math.min(100,Number(ps.microArtPositionY==null?50:ps.microArtPositionY))); return `<div class="print-art-crop print-art-${imagePosition}${artBoxMode === 'custom' ? ' print-art-custom' : ''}" style="--micro-image-width:${imageWidth}%;--micro-art-box-width:${artBoxWidth}%;--micro-art-box-height:${artBoxHeight}%;--micro-art-box-x:${artBoxX}%;--micro-art-box-y:${artBoxY}%;--micro-card-padding:${cardPadding}px;--micro-flow-gap:${flowGap}px;--micro-font-size:${microFontSize}pt;--micro-line-height:${microLineHeight}"><img src="${card._artCropImage}" alt="${escapeHtml(card.name || '')} artwork" style="object-position:${x}% ${y}%;transform:scale(${zoom});transform-origin:center"></div>`; })()
            : options.imageMode === 'embedded' && card._processedImage
              ? `<div class="print-art-crop print-art-full print-art-${imagePosition}${artBoxMode === 'custom' ? ' print-art-custom' : ''}" style="--micro-image-width:${imageWidth}%;--micro-art-box-width:${artBoxWidth}%;--micro-art-box-height:${artBoxHeight}%;--micro-art-box-x:${artBoxX}%;--micro-art-box-y:${artBoxY}%;--micro-card-padding:${cardPadding}px;--micro-flow-gap:${flowGap}px;--micro-font-size:${microFontSize}pt;--micro-line-height:${microLineHeight}"><img src="${card._processedImage}" alt="${escapeHtml(card.name || '')}"></div>`
              : '';
          const metaBits=[];
          ...
          return `<article class="print-card print-card-micro-${imagePosition}${artBoxMode === 'custom' ? ' print-card-micro-custom' : ''}" style="--micro-image-width:${imageWidth}%;--micro-art-box-width:${artBoxWidth}%;--micro-art-box-height:${artBoxHeight}%;--micro-art-box-x:${artBoxX}%;--micro-art-box-y:${artBoxY}%;--micro-card-padding:${cardPadding}px;--micro-flow-gap:${flowGap}px;--micro-font-size:${microFontSize}pt;--micro-line-height:${microLineHeight}">${microArt}<div class="print-card-copy">${header}${typeBlock}${rules}${flavor}<footer><span>${metaBits.join(' · ')}</span><strong>${ps.microShowStats===false?'':stats}</strong></footer>${price}</div></article>`;
        }).join('');
