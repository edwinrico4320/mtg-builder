      ...
      const blocks = cards.map((card, index) => {
        const img = options.imageMode === 'none' ? '' : (
          card._processedImage
            ? (card._processedImageBack
                ? `<div class="image-wrap double-faced" onclick="this.classList.toggle('flipped')"><div class="dfc-front"><img src="${card._processedImage}" alt="${escapeHtml(card.name)}"></div><div class="dfc-back"><img src="${card._processedImageBack}" alt="${escapeHtml(card.name)} back"></div></div>`
            ...
            try {
              ...
              if (resolved && resolved.dataUrl) {
                if (options.imageMode === 'art-crop') {
                  ...
                  card._processedImageBack = resolved.dataUrlBack || null;
                }
                imagesFound += 1;
              } else failures += 1;
            } catch (err) {
