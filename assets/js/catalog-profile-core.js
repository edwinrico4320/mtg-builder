    ...
    renderPrintableProfileHtml(setCode, setName, cards, options) {
      ...
      for(let pageIndex=0;pageIndex<totalPages;pageIndex++){
        ...
        const cells=slice.map((card,offset)=>{
          ...
          const microRuleLimit = microOracleMode === 'compact' ? 92 : maxRules;
          ...
          const microArt = options.imageMode === 'art-crop' && card._artCropImage
            ? (()=>{ const zoom=1+(Number(ps.microArtZoom||0)/100)*1.2; const x=Math.max(0,Math.min(100,Number(ps.microArtPositionX==null?50:ps.microArtPositionX))); const y=Math.max(0,Math.min(100,Number(ps.microArtPositionY==null?50:ps.microArtPositionY))); return `<div class="print-art-crop print-art-${imagePosition}" style="--micro-image-width:${imageWidth}%"><img src="${card._artCropImage}" alt="${escapeHtml(card.name || '')} artwork" style="object-position:${x}% ${y}%;transform:scale(${zoom});transform-origin:center"></div>`; })()
            : options.imageMode === 'embedded' && card._processedImage
              ? `<div class="print-art-crop print-art-full print-art-${imagePosition}" style="--micro-image-width:${imageWidth}%"><img src="${card._processedImage}" alt="${escapeHtml(card.name || '')}"></div>`
              : '';
          const metaBits=[];
          ...
          return `<article class="print-card print-card-micro-${imagePosition}" style="--micro-image-width:${imageWidth}%">${microArt}<div class="print-card-copy">${header}${typeBlock}${rules}${flavor}<footer><span>${metaBits.join(' · ')}</span><strong>${ps.microShowStats===false?'':stats}</strong></footer>${price}</div></article>`;
        }).join('');
