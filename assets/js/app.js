(() => {
  ...
  function generateSetHtml(data, opts){
    ...
    const cardBlocks = cards.map(c => {
      ...
      return `<article class="card" id="${c.id}"><header class="card-header"><h2>${esc(c.name)}</h2>${mana}</header>${identity}${hasFaces?'':rules}${hasFaces?'':renderStats(c)}${hasFaces?'':flavor}${faces}${details}${printings}<p class="back-link"><a href="#top">Back to top</a></p></article>`;
    }).join('\n');
