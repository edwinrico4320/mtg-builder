/*
 * MTG Builder v8.7.1 — Micro Catalog Preview
 *
...
(function () {
  ...
  function art(card, mode) {
    if (mode === 'none') return '';
    const label = mode === 'full' ? 'Full card' : 'Cropped art';
    return `<div class="mc-art mc-art-${esc(card.art)} mc-art-${esc(mode)}" aria-label="${esc(label)} preview"><span>${esc(label)}</span></div>`;
  }
  ...
  function renderCard(card, profile, index) {
