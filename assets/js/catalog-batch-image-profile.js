(function () {
  ...
  function registerModule() {
    if (typeof BuilderModules !== 'undefined') {
      BuilderModules.register('Batch Image Profile', '8.2.0');
    }
  }
...
<div class="page">
  <header class="set-header">
    ...
    <div class="set-sub">Set Code: ${escapeHtml(setCode)} · Generated by MTG Builder v8.2 · ${escapeHtml(options.profileLabel)}</div>
  </header>
  <div class="layout">
    <nav class="nav"><h2>Card Navigator</h2>${navItems}</nav>
  ...
  function selectedSetCodes() {
    const boxes = Array.from(document.querySelectorAll('#batchSetList input[type="checkbox"]:checked'));
    return boxes.map(box => {
      return box.value || box.dataset.code || box.dataset.setCode || box.getAttribute('data-code') || box.getAttribute('data-set-code') || '';
    }).filter(Boolean);
  }

  function setBatchButtonsRunning(running) {
  ...
  function setStatus(html) {
    const el = $('batchBuildStatus');
    if (el) el.innerHTML = html;
  }

  async function buildCheckedSets(ev) {
    const profile = (($('outputProfileSelect') || {}).value) || 'compact-text';
    if (profile === 'compact-text') {
      return; // let existing builder handle it
    }
    ...
    if (!setCodes.length) {
      setStatus('<p class="hint">No checked sets selected.</p>');
      return;
    }
    ...
    try {
      for (let s = 0; s < setCodes.length; s++) {
        ...
        try {
          setStatus(`<strong>Set ${s + 1} of ${setCodes.length}</strong>: Loading ${escapeHtml(setCode)}...`);
          const response = await fetch(`./data/json/${setCode}.json`);
          const json = await response.json();
          setName = getSetName(json, setCode);
          ...
          for (let i = 0; i < cards.length; i++) {
            ...
            setStatus(`<strong>Set ${s + 1} of ${setCodes.length}</strong>: ${escapeHtml(setCode)} · Card ${i + 1} of ${cards.length} · ${escapeHtml(card.name || 'Unknown card')}`);
            if (imageMode === 'embedded') {
              ...
              if (scryfallId) {
                try {
                  ...
                  failures += 1;
                  console.warn('Batch embedded image failed for', card.name, err);
                }
              } else {
                failures += 1;
              }
          ...
          buildSucceeded = !state.cancelCurrentSet;
        } catch (err) {
          totalSetsFailed += 1;
          perSetSummaries.push(`${setCode}: failed (${escapeHtml(err && err.message ? err.message : String(err))})`);
          setStatus(`<strong>Set ${s + 1} of ${setCodes.length}</strong>: ${escapeHtml(setCode)} failed. Continuing...`);
          await sleep(800);
          continue;
        }

        totalCardsProcessed += processedCards.length;
        totalImagesEmbedded += imagesFound;
        totalMissingImages += failures;
        if (buildSucceeded) totalSetsCompleted += 1;

        const report = {
          failures,
          htmlBytes
        };
        const noteWarnings = warnings(report);
        perSetSummaries.push(`${setCode}: ${processedCards.length} cards · ${imageMode === 'embedded' ? `${imagesFound} images` : 'no images'} · ${formatBytes(htmlBytes)}${noteWarnings.length ? ' · warning' : ''}${state.cancelCurrentSet ? ' · cancelled current set' : ''}`);

        setStatus(`<strong>Completed set ${s + 1} of ${setCodes.length}</strong>: ${escapeHtml(setCode)}<br>` +
          ...
          `<strong>Compatibility estimate:</strong> ${compatibility(htmlBytes, imageMode)}${noteWarnings.length ? `<div class="image-lab-warning"><strong>Warnings</strong><ul>${noteWarnings.map(w => `<li>${escapeHtml(w)}</li>`).join('')}</ul></div>` : ''}`);
        await sleep(1000);
      }

      const finalStatus = state.cancelEntireBatch ? 'Cancelled by user' : 'Complete';
      setStatus(`<strong>Batch build ${finalStatus}</strong><br>` +
        `<strong>Sets requested:</strong> ${setCodes.length}<br>` +
        `<strong>Sets completed:</strong> ${totalSetsCompleted}<br>` +
        `<strong>Sets failed:</strong> ${totalSetsFailed}<br>` +
        `<strong>Cards processed:</strong> ${totalCardsProcessed}<br>` +
        `${imageMode === 'embedded' ? `<strong>Images embedded:</strong> ${totalImagesEmbedded}<br><strong>Missing images:</strong> ${totalMissingImages}<br>` : ''}` +
        `<div class="image-lab-warning"><strong>Per-set summary</strong><ul>${perSetSummaries.map(x => `<li>${escapeHtml(x)}</li>`).join('')}</ul></div>`);
    } finally {
      state.running = false;
      state.cancelCurrentSet = false;
      state.cancelEntireBatch = false;
      setBatchButtonsRunning(false);
    }
  }
  ...
  function init() {
    ...
    if (btn) btn.addEventListener('click', buildCheckedSets, true);
