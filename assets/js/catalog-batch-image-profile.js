(function () {
  ...
  async function runBatch(setCodes, options, manifest, preloaded) {
    let totalSetsCompleted = 0, totalSetsFailed = 0, totalCardsProcessed = 0, totalImagesEmbedded = 0, totalMissingImages = 0;
    ...
    for (let s = 0; s < setCodes.length; s++) {
      ...
      try {
        ...
        perSetSummaries.push(`${setCode}: ${result.cardsProcessed} cards · ${options.imageMode==='embedded' ? `${result.imagesFound} images` : 'no images'} · ${CatalogProfileCore.formatBytes(result.htmlBytes)}${reportWarnings.length ? ' · warning' : ''}${state.cancelCurrent ? ' · cancelled current set' : ''}`);
  ...
  function init() {
    ...
    if (typeof BuilderModules !== 'undefined') BuilderModules.register('Batch Image Profile', '8.3.0');
  }
